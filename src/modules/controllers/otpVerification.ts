...
const createAlternateNumberOTP: RequestHandler = async (
  req: any,
  res: Response,
  next: NextFunction
) => {
  ...
  if (userID) {
    const { error } = validateCreateAlternateNumberOTPBody(req.body);
    if (error) {
      res.status(400).json({
...
const validateAlternateNumberOTP: RequestHandler = async (
  req: any,
  res: Response,
  next: NextFunction
) => {
  const userID = req.userId;
  if (userID) {
    const { error } = validateAlternateNumberOTPBody(req.body);
    if (error) {
      res.status(400).json({
        status: StatusMessages.ERROR,
        message: error.details[0].message,
